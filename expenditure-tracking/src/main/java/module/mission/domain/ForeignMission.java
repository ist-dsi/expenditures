...
public class ForeignMission extends ForeignMission_Base {
    ...
    public ForeignMission(final ForeignMissionProcess foreignMissionProcess,
	    final Country country, final String location, final DateTime daparture, final DateTime arrival,
	    final String objective, final Boolean useRequestingUnitAsPayingUnit,
	...
	final UpdateForeignMissionDetailsActivityInformation updateForeignMissionDetailsActivityInformation = (UpdateForeignMissionDetailsActivityInformation) updateMissionDetailsActivityInformation;
	...
	if (country == null) {
	    throw new DomainException("error.mission.country.none", ResourceBundle.getBundle("resources/MissionResources", Language.getLocale()));
	}
	if (country == MissionSystem.getInstance().getCountry()) {
	    throw new DomainException("mission.process.exception.foreign.mission.not.allowed.inside.national.country", ResourceBundle.getBundle("resources/MissionResources", Language.getLocale()));
	}
