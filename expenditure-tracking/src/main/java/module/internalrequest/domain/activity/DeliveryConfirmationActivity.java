 ...
 * 
 */
public class DeliveryConfirmationActivity
        extends InternalRequestProcessActivity<InternalRequestProcess, DeliveryConfirmationActivityInformation> {
    ...
    public boolean isActive(final InternalRequestProcess internalRequestProcess, final User user) {
        return super.isActive(internalRequestProcess, user) && InternalRequestState.DELIVERY.isPending(internalRequestProcess)
                && internalRequestProcess.canDeliver(user.getPerson());
    }

    @Override
    protected void process(final DeliveryConfirmationActivityInformation activityInformation) {
        activityInformation.getProcess().deliver(Authenticate.getUser().getPerson(), activityInformation.getDeliveryCode());
    }

    @Override
    public ActivityInformation<InternalRequestProcess> getActivityInformation(InternalRequestProcess process) {
        return new DeliveryConfirmationActivityInformation(process, this);
    }
