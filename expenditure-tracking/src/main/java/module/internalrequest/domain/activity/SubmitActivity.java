 ...
 * 
 */
public class SubmitActivity extends InternalRequestProcessActivity<InternalRequestProcess, SubmitActivityInformation> {
    ...
    public boolean isActive(final InternalRequestProcess internalRequestProcess, final User user) {
        return super.isActive(internalRequestProcess, user) && internalRequestProcess.getIsUnderConstruction()
                ...
                && !internalRequestProcess.getInternalRequest().getItemsSet().isEmpty();
    }

    @Override
    protected void process(final SubmitActivityInformation activityInformation) {
        activityInformation.getProcess().submit();
    }

    @Override
    public ActivityInformation<InternalRequestProcess> getActivityInformation(InternalRequestProcess process) {
        return new SubmitActivityInformation(process, this);
    }
