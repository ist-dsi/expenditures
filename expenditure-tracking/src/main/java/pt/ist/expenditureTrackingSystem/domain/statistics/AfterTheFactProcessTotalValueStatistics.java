		...
		for (final PaymentProcess paymentProcess : acquisitionProcessYear.getPaymentProcessSet()) {
			...
			if (paymentProcess instanceof AfterTheFactAcquisitionProcess) {
				...
				if (process.hasAcquisitionAfterTheFact() && !process.getAcquisitionAfterTheFact().getDeletedState()) {
					...
					final AfterTheFactAcquisitionType afterTheFactAcquisitionType =
							process.getAcquisitionAfterTheFact().getAfterTheFactAcquisitionType();
					result.put(afterTheFactAcquisitionType,
							result.get(afterTheFactAcquisitionType).add(process.getAcquisitionAfterTheFact().getValue()));
				}
			}
		}
	}
