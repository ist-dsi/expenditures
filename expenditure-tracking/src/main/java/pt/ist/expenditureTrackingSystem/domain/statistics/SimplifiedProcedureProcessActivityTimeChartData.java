/*
 * @(#)SimplifiedProcedureProcessActivityTimeChartData.java
 *
...
public class SimplifiedProcedureProcessActivityTimeChartData extends PaymentProcessChartData<String> {
    ...
    protected void count(final PaymentProcess paymentProcess) {
        if (paymentProcess.isSimplifiedProcedureProcess()) {
            ...
            final List<ProcessState> processStates =
                    new ArrayList<ProcessState>(simplifiedProcedureProcess.getProcessStatesSet());
            ...
            for (int i = 1; i < processStates.size(); i++) {
                ...
                if (!processState.isCanceled()) {
                    ...
                    final WorkflowLog workflowLog =
                            findLastWorkflowLog(operationLogs, previousStateChangeDateTime, stateChangeDateTime);
                    if (workflowLog != null) {
                        final WorkflowActivity abstractActivity =
                                getActivity(simplifiedProcedureProcess, (ActivityLog) workflowLog);
    ...
    private WorkflowLog findLastWorkflowLog(final List<WorkflowLog> orderedOperationLogs, final DateTime start, final DateTime end) {
