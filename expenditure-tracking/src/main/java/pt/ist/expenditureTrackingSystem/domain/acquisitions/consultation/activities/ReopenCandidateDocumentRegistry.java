...
public class ReopenCandidateDocumentRegistry extends WorkflowActivity<MultipleSupplierConsultationProcess, ActivityInformation<MultipleSupplierConsultationProcess>> {

    @Override
    public boolean isActive(final MultipleSupplierConsultationProcess process, final User user) {
        return process.getState() == MultipleSupplierConsultationProcessState.PENDING_EVALUATION
                && (ExpenditureTrackingSystem.isAcquisitionCentralGroupMember(user) || process.getConsultation().isJuryMember(user));
    }
    ...
    protected void process(final ActivityInformation<MultipleSupplierConsultationProcess> information) {
        ...
        process.setState(MultipleSupplierConsultationProcessState.PENDING_CANDIDATE_DOCUMENTATION);
    }
