...
<div class="page-body">
    ...
    <form class="form-horizontal" action="<%= request.getContextPath() %>/missions/config/addQueueForAccountabilityType" method="POST">
        ${csrf.field()}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <select name="workflowQueue" class="form-control" required="required">
                    <option>--</option>
                    <c:forEach items="${workflowQueues}" var="queue">
                        <option value="${queue.externalId}">${queue.name}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
