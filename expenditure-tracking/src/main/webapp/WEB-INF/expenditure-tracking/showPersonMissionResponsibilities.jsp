...
<div>
	...
	%>
			<h3>
				<spring:message code="label.module.mission.person.mission.responsibilities"
				...
				<table class="table" id="personMissionResponsability">
					...
					<tbody id="searchResultsRespPerson">
						...
						%>
								<tr class="ng-scope">
									<td>
										<a href="<%=contextPath%>/expenditure-tracking/manageMissions/?partyId=<%=authorityAccountability.getParent().getExternalId()%>"
												class="" title=""> <%=authorityAccountability.getParent().getPresentationName()%></a>
									</td>
									<td>
									...
									<td class="ng-binding">
										<%=authorityAccountability.getEndDate() == null ? "" : authorityAccountability.getEndDate()%>
									</td>
									<%
										if (ExpenditureTrackingSystem.getInstance().getAcquisitionsUnitManagerGroup().isMember(Authenticate.getUser())) {
									%>
											<td>
												...
												<% } %>
											<% if(closerr && linha.equals(authorityAccountability.getExternalId())) {%>
													<span class="error0">
														<spring:message code="error.mission.begin.is.after.end" text="true"/>
													</span>
											<% } %>
											</td>
									<% } %>
