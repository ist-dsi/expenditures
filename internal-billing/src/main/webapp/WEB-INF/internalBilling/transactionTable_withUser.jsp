...
<table class="table">
    <thead>
        <tr>
            ...
            <th width="35%">
                <spring:message code="label.equitrac.entry.user" text="User"/>
            </th>
        </tr>
    </thead>
    <tbody>
            ...
            <tr <% if (value.intValue() > 10) { %> style="background-color: #ffaaaa;" <% } else if (value.intValue() > 5) { %> style="background-color: #ffd4aa; "<% } %>>
                ...
                <td>
                    <img class="img-circle" width="30" height="30" alt=""
                            src="<%= tx.getUser().getProfile().getAvatarUrl() %>"/>
                        <a href="<%= contextPath + "/internalBilling/user/" + tx.getUser().getExternalId() %>">
                            <%= tx.getUser().getProfile().getDisplayName() %>
                        </a>
                </td>
            </tr>
        <% } %>   
